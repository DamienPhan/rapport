...
export function enrichWithPhones(missions, fullText, cfg) {
  missions.forEach((m) => {
    if (!m.greeteurPhone && m.greeteur) {
      ...
      if (r) {
        // Fenêtre bornée par la prochaine réf booking (comme contactPhone
        // ci-dessous) : sans borne, un greeter sans téléphone propre (label
        // présent mais rien après) récupérait le numéro de la mission
        // suivante — ex. « Greeter: Antoine » (pas de tel) suivi 150
        // caractères plus loin par « Bryan L 06 10 88 78 90 », faussement
        // assigné à Antoine.
        const afterMatch = fullText.slice(r.index + r[0].length);
        const nextRef = afterMatch.match(/\b\d{4,6}-\d+\b/);
        const windowEnd = nextRef ? Math.min(nextRef.index, 200) : 200;
        // Aplatit les caractères après le nom (numéros scindés sur 2 lignes, ex. "+33 6 47\n48 64 66")
        const flat = afterMatch.slice(0, windowEnd).replace(/\n/g, ' ');
